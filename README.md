...
1. Apps Script エディタで `showApiToken()` を実行し、実行ログに出たトークンを控える
2. Claude Code の環境設定で環境変数 `TASKBOARD_TOKEN` にそのトークンを設定する

**トークンはリポジトリに置かないこと。** `docs/config.js` は GitHub Pages で公開されるので、
そこに書くと誰でも書き込めるようになる。漏れた時は `regenerateApiToken()` を実行すれば
古いトークンは即座に無効になる。
