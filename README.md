...
│  ・JSON登録             │        │  ・オフラインで開ける         │
│  ・原本画像             │        │  ・日本語大／英語小            │
│  ・スケッチ             │        │  ・AXMER 全21タスクの解説     │
└───────────┬─────────────┘        └──────────────┬───────────────┘
            │ google.script.run                   │ GET (JSON / JSONP)
            ▼                                     ▼
     ┌──────────────────────────────────────────────────┐
     │  スプレッドシート（data / image / sketch_*）      │
     └──────────────────────────────────────────────────┘
```
...
| パス | 役割 |
|---|---|
| `コード.js` | GAS。JSON API（`?action=data/image/sketch/ping`）と管理画面の配信 |
| `index.html` | GAS の入力・管理画面 |
| `docs/` | GitHub Pages に公開する閲覧用PWA |
| `docs/app.js` | 正規化・辞書適用・描画 |
...
3. 出てきたJSONを貼り付けて「登録して全クルーに反映」
4. 原本画像・スケッチがあれば登録
5. 「クルーに配るリンク」をLINEで共有

### クルー

1. 配られたURLを開く（ホーム画面に追加しておくとアプリとして起動する）
2. **電波のあるうちに一度「↻」を押す** — これで端末に保存され、圏外でも開ける
3. タスク名の横の **?** でそのタスクのAXMERルール（和訳）が読める
